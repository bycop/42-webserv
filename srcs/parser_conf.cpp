...
void display_error(std::string const& type, std::string const& key) {
	cerr << "\e[91m[ERROR]\e[0m Unrecognized " << type << " key at '"
			  << key << "'" << std::endl;
	error = true;
}

void setters(std::string const& newdata, const int type, Data &data) {
	switch (type) {
		case 0:
...
void server_setters(std::string const& newdata, const int type, Server &server) {
	switch (type) {
		case 0:
			server.host = newdata;
			break;
...
void location_setters(std::string const& newdata, const int type, Location &location) {
	switch (type) {
		case 0:
...
std::string location_path(std::string const& line) {
...
void location_loop(std::ifstream &file, Server &server, std::string const& path) {
	Location location;
	std::string line;
...
int parser_conf(Data &data, string const& file_path) {
	...
	if (error || !data.mandatory()) {
		cerr << "Error: Exiting program" << std::endl;
		return (1);
	}
	return (0);
};