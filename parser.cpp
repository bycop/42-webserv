...
void location_loop(std::ifstream &file, Server *server) {
	...
	while (std::getline(file, line)) {
		...
		if (line.find("}", 0) == 0)
			...
			std::cout << "\e[93m[WARNING]\e[0m Unrecognized location key at '" << original << "'" << std::endl;
	}
...
void server_loop(std::ifstream &file, Data *data) {
	Server server;
	std::string line;
	std::string servervars[5] = {"host", "port", "server_name", "default_pages", "size_limit"};
	bool find;
	...
	while (std::getline(file, line)) {
		...
		if (!find && line.find("location{", 0) == 0)
			...
			std::cout << "\e[93m[WARNING]\e[0m Unrecognized server key at '" << original << "'" << std::endl;
	}
...
int main(int ac, char **av) {
	...
	while (std::getline(file, line)) {
		...
		if (!find && line.find("server{", 0) == 0)
			...
			std::cout << "\e[93m[WARNING]\e[0m Unrecognized main key at '" << original << "'" << std::endl;
	}
